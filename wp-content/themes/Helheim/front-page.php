...
<main>
  ...
  <section id="blog-accueil">
      ...
      $i = 0;
      $bool = 0;
      $query = new WP_Query(array('category_name'=>'article', 'posts_per_page'=>'5', 'order'=>'DESC'));
      while ($query->have_posts()){
        $query->the_post();
        if($i == 4 ){
          break;
        }
        if ($bool == 0 &&  has_category('Design', get_the_ID() )){
          $bool = 1;
          $i = $i-1;
        }else{
          $i=$i+1; 
      ...
      <p class="home-link"><a href="/blog">Voir tout les articles >></a></p>
  </section>
</main>
<?php get_footer() ?>